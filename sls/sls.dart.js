...
$$.BoundClosure$i1 = [V, {"": "BoundClosure;_self,_target,_receiver,__js_helper$_name",
  call$1: function(p0) {
...
Interceptor: {"": "Object;",
  ...
  "%": "CanvasGradient|CanvasPattern|CanvasRenderingContext|CanvasRenderingContext2D|DOMError|FileError|MediaError|MediaKeyError|Navigator|NavigatorUserMediaError|PositionError|SQLError|SVGAnimatedLength|SVGAnimatedNumberList"
},
...
TextAreaElement: {"": "HtmlElement;rows=", "%": "HTMLTextAreaElement"},
...
["plot", "package:sls/src/plot.dart", , V, {
Plot: {"": "Object;canvas,context,width,height,x_scale,y_scale,extents,x_min,y_min,x_max,y_max,x_range,y_range,x_origin,y_origin,major_tick_size,minor_tick_size,units_per_tick,axis_width,axis_color,axis_font",
  draw_x_axis$0: function() {
    var unit, x_step, t1, x_position, unit0, t2, t3, t4;
    unit = this.units_per_tick;
    x_step = unit * this.x_scale;
    t1 = this.context;
    t1.save();
    t1.beginPath();
    t1.lineWidth = this.axis_width;
    t1.strokeStyle = this.axis_color;
    t1.font = this.axis_font;
    t1.textAlign = "center";
    t1.textBaseline = "top";
    t1 = this.context;
    t1.moveTo(0, this.y_origin);
    t1.lineTo(this.width, this.y_origin);
    t1.stroke();
    x_position = this.x_origin - x_step;
    unit0 = -unit;
    for (t1 = this.major_tick_size / 2; x_position > 0;) {
      t2 = this.context;
      t2.moveTo(x_position, this.y_origin - t1);
      t2.lineTo(x_position, this.y_origin + t1);
      t2.stroke();
      t3 = C.JSInt_methods.toString$0(unit0);
      t4 = this.y_origin;
      t2.fillText(t3, x_position, t4 + t1 + 3);
      unit0 -= unit;
      x_position -= x_step;
    }
    x_position = this.x_origin + x_step;
    ...
    while (true) {
      ...
      if (!(x_position < t2))
        break;
      t3.moveTo(x_position, this.y_origin - t1);
      t3.lineTo(x_position, this.y_origin + t1);
      t3.stroke();
      t2 = C.JSInt_methods.toString$0(unit0);
      t4 = this.y_origin;
      t3.fillText(t2, x_position, t4 + t1 + 3);
      unit0 += unit;
      x_position += x_step;
    }
    t3.closePath();
  ...
  draw_y_axis$0: function() {
    var unit, y_step, t1, y_position, unit0, t2, t3, t4;
    unit = this.units_per_tick;
    y_step = unit * this.y_scale;
    t1 = this.context;
    t1.save();
    t1.beginPath();
    t1.lineWidth = this.axis_width;
    ...
    t1 = this.context;
    t1.moveTo(this.x_origin, 0);
    t1.lineTo(this.x_origin, this.height);
    t1.stroke();
    y_position = this.y_origin - y_step;
    for (t1 = this.major_tick_size / 2, unit0 = unit; y_position > 0;) {
      t2 = this.context;
      t2.moveTo(this.x_origin - t1, y_position);
      t2.lineTo(this.x_origin + t1, y_position);
      t2.stroke();
      t3 = C.JSInt_methods.toString$0(unit0);
      t4 = this.x_origin;
      t2.fillText(t3, t4 - t1 - 3, y_position);
      unit0 += unit;
      y_position -= y_step;
    }
    y_position = this.y_origin + y_step;
    ...
    while (true) {
      ...
      if (!(y_position < t2))
        break;
      t3.moveTo(this.x_origin - t1, y_position);
      t3.lineTo(this.x_origin + t1, y_position);
      t3.stroke();
      t2 = C.JSInt_methods.toString$0(unit0);
      t4 = this.x_origin;
      t3.fillText(t2, t4 - t1 - 3, y_position);
      unit0 -= unit;
      y_position += y_step;
    }
    t3.closePath();
  ...
  plot_point$4: function(x, y, color, size) {
    var x_position, t1, t2;
    x_position = J.$add$ns(J.$mul$n(x, this.x_scale), this.x_origin);
    t1 = J.$sub$n(J.$mul$n(y, this.y_scale), this.y_origin);
    if (typeof t1 !== "number")
      throw t1.$negate();
    t2 = this.context;
    t2.save();
    ...
    t2 = this.context;
    t2.beginPath();
    t2.arc(x_position, -t1, size / 2, 0, 6.283185307179586, false);
    t2.fill();
    t2.stroke();
    t2.closePath();
    this.context.restore();
  },
  plot_point$3: function(x, y, color) {
    return this.plot_point$4(x, y, color, 10);
  },
  plot_points$4: function($X, $Y, color, size) {
    var i, t1;
    for (i = 0; i < $X.length; ++i) {
      t1 = $X[i];
      if (i >= $Y.length)
        throw H.ioore($Y, i);
      this.plot_point$4(t1, $Y[i], color, size);
    }
  },
  plot_points$3: function(X, Y, color) {
    return this.plot_points$4(X, Y, color, 10);
  },
  plot_function$3: function(f, color, thickness) {
    ...
    t4 = this.x_origin;
    t5 = t1[1];
    t6 = J.$sub$n(J.$mul$n(f.call$1(t2), this.y_scale), this.y_origin);
    if (typeof t6 !== "number")
      throw t6.$negate();
    t1 = J.$sub$n(J.$mul$n(f.call$1(t1[1]), this.y_scale), this.y_origin);
    if (typeof t1 !== "number")
      throw t1.$negate();
    t7 = this.context;
    t7.save();
    t7.strokeStyle = color;
    t7.lineWidth = thickness;
    t7 = this.context;
    t7.beginPath();
    t7.moveTo(t2 * t3 + t4, -t6);
    t7.lineTo(t5 * t3 + t4, -t1);
    t7.stroke();
    t7.closePath();
    this.context.restore();
  },
  plot_function$2: function(f, color) {
    return this.plot_function$3(f, color, 1);
  },
  clear$1: function(_, _0) {
    var t1 = this.context;
    t1.save();
    t1.fillStyle = "white";
    t1.beginPath();
    t1.rect(0, 0, this.width, this.height);
    t1.fill();
    t1.closePath();
    t1.restore();
    this.draw_x_axis$0();
  ...
  get$clear: function(_receiver) {
    return new V.BoundClosure$i1(this, V.Plot.prototype.clear$1, _receiver, "clear$1");
  },
  Plot$3: function(canvas, extents, units_per_tick) {
    var t1, t2;
    t1 = this.canvas;
    this.context = J.get$context2D$x(t1);
    this.width = t1.width;
    ...
    this.y_min = t1[3];
    this.y_max = t1[2];
    this.x_range = Math.abs(this.x_max - this.x_min);
    this.y_range = Math.abs(this.y_max - this.y_min);
    t1 = this.width;
    t2 = this.x_range;
    if (typeof t1 !== "number")
      throw t1.$div();
    ...
    if (typeof t2 !== "number")
      throw t2.$div();
    this.y_scale = t2 / t1;
    t1 = Math.abs(this.x_min / this.x_range);
    t2 = this.width;
    if (typeof t2 !== "number")
      throw H.iae(t2);
    this.x_origin = t1 * t2;
    t2 = Math.abs(this.y_min / this.y_range);
    t1 = this.height;
    if (typeof t1 !== "number")
      throw H.iae(t1);
    this.y_origin = t2 * t1;
    t1 = window;
    t2 = this.get$clear(this);
    C.Window_methods._ensureRequestAnimationFrame$0(t1);
    C.Window_methods._requestAnimationFrame$1(t1, W._wrapZone(t2));
  },
  static: {
Plot$: function(canvas, extents, units_per_tick) {
  var t1 = new V.Plot(canvas, null, null, null, null, null, extents, null, null, null, null, null, null, null, null, 20, 5, units_per_tick, 1, "black", "8pt Consolas");
  t1.Plot$3(canvas, extents, units_per_tick);
  return t1;
}}

},

PlotAnimation: {"": "Object;",
  animate$1: function(_) {
    var t1, t2;
    t1 = this.queue;
    if (t1._head !== t1._tail) {
      ...
      t1 = this.delay;
      if (t1 > 0)
        P.Timer_Timer(P.Duration$(0, 0, 0, t1, 0, 0), new V.PlotAnimation_animate_closure(this));
      else {
        t1 = window;
        t2 = this.get$animate();
        C.Window_methods._ensureRequestAnimationFrame$0(t1);
  ...
  get$animate: function() {
    return new H.BoundClosure$1(this, V.PlotAnimation.prototype.animate$1, null, "animate$1");
  }
},

PlotAnimation_animate_closure: {"": "Closure;this_0",
  call$0: function() {
    var t1, t2;
    t1 = window;
    t2 = this.this_0.get$animate();
    C.Window_methods._ensureRequestAnimationFrame$0(t1);
    C.Window_methods._requestAnimationFrame$1(t1, W._wrapZone(t2));
  }
}}],
["random_util", "package:sls/src/random_util.dart", , M, {
random_color: function() {
  var r, g, b;
  r = C.JSDouble_methods.toInt$0(Math.random() * 255 + 0);
  g = C.JSDouble_methods.toInt$0(Math.random() * 255 + 0);
  b = C.JSDouble_methods.toInt$0(Math.random() * 255 + 0);
  return "rgb(" + r + ", " + g + ", " + b + ")";
}}],
["sls", "package:sls/src/least_squares.dart", , F, {
least_squares: function($X, $Y) {
  var n, t1, x_mean, y_mean, i, t2, x_variance, y_variance, covariance, x_deviation, y_deviation, slope;
  n = $X.length;
  if (n < 1)
    throw H.wrapException(P.RangeError$("Empty list."));
  t1 = $Y.length;
  if (t1 !== n)
    throw H.wrapException(P.RangeError$("Vector length mismatch."));
  for (x_mean = 0, y_mean = 0, i = 0; i < n; ++i) {
    t2 = $X[i];
    if (typeof t2 !== "number")
      throw H.iae(t2);
    x_mean += t2;
  ...
  x_mean /= n;
  y_mean /= n;
  for (x_variance = 0, y_variance = 0, covariance = 0, i = 0; i < n; ++i) {
    if (i >= $X.length)
      throw H.ioore($X, i);
    x_deviation = J.$sub$n($X[i], x_mean);
    if (i >= $Y.length)
      throw H.ioore($Y, i);
    y_deviation = J.$sub$n($Y[i], y_mean);
    t1 = J.getInterceptor$n(x_deviation);
    t2 = t1.$mul(x_deviation, x_deviation);
    if (typeof t2 !== "number")
      throw H.iae(t2);
    x_variance += t2;
    t2 = J.$mul$n(y_deviation, y_deviation);
    if (typeof t2 !== "number")
      throw H.iae(t2);
    y_variance += t2;
    t1 = t1.$mul(x_deviation, y_deviation);
    if (typeof t1 !== "number")
      throw H.iae(t1);
    covariance += t1;
  }
  ...
  return new F.LeastSquaresLine(slope, y_mean - slope * x_mean, t1 * (y_variance / t1 - covariance * covariance / x_variance), null, null);
},

LinearEquation: {"": "Object;slope<,y_intercept<",
  to_function$0: function() {
    return new F.LinearEquation_to_function_closure(this);
  },
  toString$0: function(_) {
    var t1, s, y;
    t1 = this.get$slope();
    if (t1 != null && this.get$y_intercept() != null) {
      s = J.toStringAsFixed$1$n(t1, 3);
      t1 = this.get$y_intercept();
      if (typeof t1 !== "number")
        throw t1.$gt();
      if (t1 > 0) {
        y = C.JSNumber_methods.toStringAsFixed$1(t1, 3);
        return "y = " + s + "x + " + y;
      } else if (t1 < 0) {
        y = C.JSNumber_methods.toStringAsFixed$1(-t1, 3);
        return "y = " + s + "x - " + y;
...
LinearEquation_to_function_closure: {"": "Closure;this_0",
  call$1: function(n) {
    var t1, t2;
    t1 = this.this_0;
    t2 = t1.get$slope();
...
LeastSquaresLine: {"": "LinearEquation;slope<,y_intercept<,sse<,sls$LinearEquation$slope,sls$LinearEquation$y_intercept",
  toString$0: function(_) {
    var t1, s, e, y;
    t1 = this.slope;
    if (t1 != null && this.y_intercept != null) {
      s = J.toStringAsFixed$1$n(t1, 3);
      e = C.JSNumber_methods.toStringAsFixed$1(this.sse, 3);
      t1 = this.y_intercept;
      if (typeof t1 !== "number")
        throw t1.$gt();
      if (t1 > 0) {
        y = C.JSNumber_methods.toStringAsFixed$1(t1, 3);
        return "y = " + s + "x + " + y + ", SSE = " + e;
      } else if (t1 < 0) {
        y = C.JSNumber_methods.toStringAsFixed$1(-t1, 3);
        return "y = " + s + "x - " + y + ", SSE = " + e;
      } else
      ...
      return "single point, SSE = 0";
  }
},

SLS: {"": "Object;X,Y,M,lines,cost,segments,C,length>",
  calc_lines$0: function() {
    var t1, i, t2, j, j0, t3, t4;
    t1 = P.List_List(this.length, [J.JSArray, F.LeastSquaresLine]);
    H.setRuntimeTypeInfo(t1, [[J.JSArray, F.LeastSquaresLine]]);
    this.lines = t1;
    for (i = 0; t1 = this.length, i < t1; ++i) {
      t2 = this.lines;
      t1 = P.List_List(t1, F.LeastSquaresLine);
      t1.$builtinTypeInfo = [F.LeastSquaresLine];
      if (i >= t2.length)
        throw H.ioore(t2, i);
      t2[i] = t1;
    }
    for (t1 = this.X, t2 = this.Y, j = 0; j < this.length; j = j0)
      for (j0 = j + 1, i = 0; i < j0; ++i) {
        t3 = this.lines;
        if (i === j) {
          if (i >= t3.length)
            throw H.ioore(t3, i);
          ...
          if (j >= t3.length)
            throw H.ioore(t3, j);
          t3[j] = new F.LeastSquaresLine(null, null, 0, null, null);
        } else {
          if (i >= t3.length)
            throw H.ioore(t3, i);
          t3 = t3[i];
          t4 = F.least_squares(C.JSArray_methods.sublist$2(t1, i, j0), C.JSArray_methods.sublist$2(t2, i, j0));
          if (j >= t3.length)
            throw H.ioore(t3, j);
          t3[j] = t4;
        }
  ...
  optimal$0: function() {
    var t1, i, t2, j, t3, t4, low_cost, j0, t5;
    t1 = P.List_List(this.length + 1, J.JSDouble);
    H.setRuntimeTypeInfo(t1, [J.JSDouble]);
    this.M = t1;
    t1 = P.List_List(this.length, [J.JSArray, J.JSDouble]);
    H.setRuntimeTypeInfo(t1, [[J.JSArray, J.JSDouble]]);
    this.cost = t1;
    for (i = 0; t1 = this.length, i < t1; ++i) {
      t2 = this.cost;
    ...
    if (0 >= t1.length)
      throw H.ioore(t1, 0);
    t1[0] = 0;
    for (t1 = this.C, j = 0; j < this.length; j = j0) {
      t2 = this.cost;
      if (0 >= t2.length)
        throw H.ioore(t2, 0);
      t3 = t2[0];
      ...
      if (j >= t2.length)
        throw H.ioore(t2, j);
      low_cost = t2[j];
      for (j0 = j + 1, i = 0; t2 = this.M, i < j0; ++i) {
        t3 = this.cost;
        if (i >= t3.length)
          throw H.ioore(t3, i);
        t4 = t3[i];
  ...
  find_solution$0: function() {
    var j, t1, low_cost, cost, t2, i, t3, t4, c, low_i, points;
    this.segments = [];
    j = this.length - 1;
    if (j <= 0)
      return [];
    else {
      for (t1 = this.C, low_cost = null; j > 0;) {
        cost = [];
        for (t2 = j + 1, i = 0; i < t2; ++i) {
          t3 = this.lines;
          if (i >= t3.length)
            throw H.ioore(t3, i);
          t3 = t3[i];
        ...
        if (t3 !== 0) {
          if (0 >= t3)
            throw H.ioore(cost, 0);
          low_cost = cost[0];
          for (t3 = new H.ListIterator(cost, t3, 0, null); t3.moveNext$0();) {
            c = t3._current;
            if (J.$lt$n(c, low_cost))
              low_cost = c;
          }
          low_i = H.Arrays_indexOf(cost, low_cost, 0, cost.length);
        } else
          low_i = 0;
        points = [];
        for (i = low_i; i < t2; ++i)
  ...
  toString$0: function(_) {
    return J.toString$0(this.segments);
  }
}}],
["", "sls.dart", , F, {
main: function() {
  ...
  for (i = 0; i < 10; ++i) {
    $X.push(i);
    $Y.push(F.f2(i) + (Math.random() * 0.7 + -0.35));
  }
  F.test_segmented_least_squares($X, $Y, 0.6, [-1, 10, -2, 10], 50);
},
...
test_segmented_least_squares: function($X, $Y, $C, extents, delay) {
  var p, sls, i, t1, start, end, line;
  p = V.Plot$(document.querySelector("#plot_canvas"), extents, 1);
  sls = new F.SLS($X, $Y, null, null, null, null, $C, null);
  sls.length = sls.X.length;
  sls.calc_lines$0();
  sls.optimal$0();
  sls.find_solution$0();
  new F.SLSAnimation(sls, P.ListQueue$(null, P.Function), p, delay).run$0();
  P.print("Test Segmented Least Squares:");
  for (i = sls.segments.length - 1; i >= 0; --i) {
    t1 = sls.segments;
    if (i >= t1.length)
      throw H.ioore(t1, i);
    t1 = t1[i];
    if (0 >= t1.length)
      throw H.ioore(t1, 0);
    start = t1[0];
    end = C.JSArray_methods.get$last(t1);
    t1 = sls.lines;
    if (start >>> 0 !== start || start >= t1.length)
      throw H.ioore(t1, start);
    t1 = t1[start];
    if (end >>> 0 !== end || end >= t1.length)
      throw H.ioore(t1, end);
    line = t1[end];
    line = "Equation for points " + (start + 1) + ".." + (end + 1) + ": " + H.S(line);
    H.printToConsole(line);
  }
},

SLSAnimation: {"": "PlotAnimation;sls,queue,plot,delay",
  run$0: function() {
    var t1, t2;
    this.queue._add$1(new F.SLSAnimation_run_closure(this));
    t1 = this.sls;
    t1.calc_lines$0();
    t1.optimal$0();
    t1.find_solution$0();
    this.initialize_table$1("#error_table");
    this.initialize_table$1("#cost_table");
    this.initialize_opt_table$0();
    this.error_animation$0();
    this.optimal_animation$0();
    this.plot_solution$0();
    t1 = window;
    t2 = this.get$animate();
    C.Window_methods._ensureRequestAnimationFrame$0(t1);
    C.Window_methods._requestAnimationFrame$1(t1, W._wrapZone(t2));
  },
  initialize_table$1: function(id) {
    var table, row, cell, t1, j, i;
    table = document.querySelector(id);
    row = document.createElement("tr", null);
    cell = document.createElement("td", null);
    cell.textContent = "i \\ j";
    row.appendChild(cell);
    for (t1 = this.sls, j = 0; j < t1.length;) {
      cell = document.createElement("td", null);
      ++j;
      cell.textContent = C.JSInt_methods.toString$0(j);
      row.appendChild(cell);
    }
    ...
    for (i = 0; i < t1.length;) {
      ...
      cell = document.createElement("td", null);
      ++i;
      cell.textContent = C.JSInt_methods.toString$0(i);
      row.appendChild(cell);
      for (j = 0; j < t1.length; ++j)
        row.appendChild(document.createElement("td", null));
      table.appendChild(row);
    }
  },
  initialize_opt_table$0: function() {
    var table, row_1, cell_1, row_2, cell_2, t1, j;
    table = document.querySelector("#opt_table");
    row_1 = document.createElement("tr", null);
    cell_1 = document.createElement("td", null);
    row_2 = document.createElement("tr", null);
    cell_2 = document.createElement("td", null);
    cell_1.textContent = "j";
    row_1.appendChild(cell_1);
    cell_2.textContent = "opt[j]";
    row_2.appendChild(cell_2);
    for (t1 = this.sls, j = 0; j <= t1.length; ++j) {
      cell_1 = document.createElement("td", null);
      cell_1.textContent = C.JSInt_methods.toString$0(j);
      row_1.appendChild(cell_1);
      row_2.appendChild(document.createElement("td", null));
    }
    table.appendChild(row_1);
    table.appendChild(row_2);
    t1 = J.get$cells$x(J.$index$asx(J.get$rows$x(table), 1))._list;
    if (1 >= t1.length)
      throw H.ioore(t1, 1);
    t1[1].textContent = C.JSInt_methods.toStringAsFixed$1(0, 2);
  },
  error_animation$0: function() {
    var error_table, box_1, t1, t2, t3, box_0, t4, t5, box_00, cell;
    error_table = document.querySelector("#error_table");
    box_1 = {};
    for (box_1.j_1 = 0, t1 = this.sls, t2 = J.getInterceptor$x(error_table), t3 = this.queue; box_1.j_1 < t1.length; box_1 = {}, box_1.j_1 = t5, box_1.j_1 = box_1.j_1 + 1) {
      t3._add$1(new F.SLSAnimation_error_animation_closure(box_1, this));
      box_0 = {};
      for (box_0.i_0 = 0; t4 = box_0.i_0, t5 = box_1.j_1, t4 < t5 + 1; box_00 = {}, box_00.i_0 = box_0.i_0, box_00.i_0 = box_00.i_0 + 1, box_0 = box_00) {
        t4 = box_1.j_1 + 1;
        t5 = J.get$cells$x(J.$index$asx(t2.get$rows(error_table), box_0.i_0 + 1))._list;
        if (t4 < 0 || t4 >= t5.length)
          throw H.ioore(t5, t4);
        cell = t5[t4];
        t3._add$1(new F.SLSAnimation_error_animation_closure0(box_0, box_1, this, cell));
        t3._add$1(new F.SLSAnimation_error_animation_closure1(box_0, this, cell));
      }
    }
  },
  optimal_animation$0: function() {
    var cost_table, opt_table, error_table, box_1, t1, t2, t3, t4, t5, box_10, box_0, box_00, t6, t7, cost_cell, previous_cost_cell, error_cell, opt_cell;
    cost_table = document.querySelector("#cost_table");
    opt_table = document.querySelector("#opt_table");
    error_table = document.querySelector("#error_table");
    box_1 = {};
    for (box_1.j_1 = 0, t1 = this.sls, t2 = J.getInterceptor$x(opt_table), t3 = this.queue, t4 = J.getInterceptor$x(cost_table), t5 = J.getInterceptor$x(error_table); box_1.j_1 < t1.length; box_10 = {}, box_10.j_1 = box_1.j_1, box_10.j_1 = box_10.j_1 + 1, box_1 = box_10) {
      t3._add$1(new F.SLSAnimation_optimal_animation_closure(box_1, this));
      box_0 = {};
      for (box_0.i_0 = 0; box_0.i_0 < box_1.j_1 + 1; box_00 = {}, box_00.i_0 = box_0.i_0, box_00.i_0 = box_00.i_0 + 1, box_0 = box_00) {
        t6 = box_1.j_1 + 1;
        t7 = J.get$cells$x(J.$index$asx(t4.get$rows(cost_table), box_0.i_0 + 1))._list;
        if (t6 < 0 || t6 >= t7.length)
          throw H.ioore(t7, t6);
        cost_cell = t7[t6];
        t6 = box_0.i_0 + 1;
        t7 = J.get$cells$x(J.$index$asx(t2.get$rows(opt_table), 1))._list;
        if (t6 < 0 || t6 >= t7.length)
          throw H.ioore(t7, t6);
        previous_cost_cell = t7[t6];
        t6 = box_1.j_1 + 1;
        t7 = J.get$cells$x(J.$index$asx(t5.get$rows(error_table), box_0.i_0 + 1))._list;
        if (t6 < 0 || t6 >= t7.length)
          throw H.ioore(t7, t6);
        error_cell = t7[t6];
        t3._add$1(new F.SLSAnimation_optimal_animation_closure0(box_0, box_1, this, cost_cell, previous_cost_cell, error_cell));
        t3._add$1(new F.SLSAnimation_optimal_animation_closure1(box_0, this, cost_cell, previous_cost_cell, error_cell));
      }
      t6 = box_1.j_1 + 2;
      t7 = J.get$cells$x(J.$index$asx(t2.get$rows(opt_table), 1))._list;
      if (t6 < 0 || t6 >= t7.length)
        throw H.ioore(t7, t6);
      opt_cell = t7[t6];
      t3._add$1(new F.SLSAnimation_optimal_animation_closure2(box_1, this, opt_cell));
      t3._add$1(new F.SLSAnimation_optimal_animation_closure3(opt_cell));
    }
  },
  plot_solution$0: function() {
    var equation_list, cost_table, opt_table, t1, t2, t3, t4, t5, t6, i, t7, start, end, X0, Y0, line, t8, opt_cell, t9, cost_cell, i_cell;
    equation_list = document.querySelector("#equation_list");
    cost_table = document.querySelector("#cost_table");
    opt_table = document.querySelector("#opt_table");
    for (t1 = this.sls, t2 = t1.X, t3 = t1.Y, t4 = J.getInterceptor$x(opt_table), t5 = J.getInterceptor$x(cost_table), t6 = this.queue, i = 0; t7 = t1.segments, i < t7.length; ++i) {
      t7 = t7[i];
      if (0 >= t7.length)
        throw H.ioore(t7, 0);
      start = t7[0];
      end = C.JSArray_methods.get$last(t7);
      t7 = J.getInterceptor$ns(end);
      X0 = C.JSArray_methods.sublist$2(t2, start, t7.$add(end, 1));
      Y0 = C.JSArray_methods.sublist$2(t3, start, t7.$add(end, 1));
      t7 = t1.lines;
      if (start >>> 0 !== start || start >= t7.length)
        throw H.ioore(t7, start);
      t7 = t7[start];
      if (end >>> 0 !== end || end >= t7.length)
        throw H.ioore(t7, end);
      line = t7[end];
      t7 = end + 2;
      t8 = J.get$cells$x(J.$index$asx(t4.get$rows(opt_table), 1))._list;
      if (t7 >= t8.length)
        throw H.ioore(t8, t7);
      opt_cell = t8[t7];
      t7 = start + 1;
      t8 = end + 1;
      t9 = J.get$cells$x(J.$index$asx(t5.get$rows(cost_table), t7))._list;
      if (t8 >= t9.length)
        throw H.ioore(t9, t8);
      cost_cell = t9[t8];
      t7 = J.get$cells$x(J.$index$asx(t5.get$rows(cost_table), t7))._list;
      if (0 >= t7.length)
        throw H.ioore(t7, 0);
      i_cell = t7[0];
      t7 = J.get$cells$x(J.$index$asx(t5.get$rows(cost_table), 0))._list;
      if (t8 >= t7.length)
        throw H.ioore(t7, t8);
      t6._add$1(new F.SLSAnimation_plot_solution_closure(this, equation_list, X0, Y0, line, opt_cell, cost_cell, i_cell, t7[t8], M.random_color()));
    }
  }
},

SLSAnimation_run_closure: {"": "Closure;this_0",
  call$0: function() {
    var t1, t2;
    t1 = this.this_0;
    t2 = t1.plot;
    t2.clear$1(t2, null);
    t1 = t1.sls;
    t2.plot_points$3(t1.X, t1.Y, "blue");
  }
},

SLSAnimation_error_animation_closure: {"": "Closure;box_1,this_2",
  call$0: function() {
    var t1, t2, t3, t4;
    t1 = this.this_2;
    t2 = t1.sls;
    ...
    if (t4 >= t2.length)
      throw H.ioore(t2, t4);
    t1.plot.plot_point$3(t3, t2[t4], "green");
  }
},

SLSAnimation_error_animation_closure0: {"": "Closure;box_0,box_1,this_3,cell_4",
  call$0: function() {
    var t1, t2, t3, t4, t5, t6, t7;
    t1 = this.this_3;
    t2 = t1.plot;
    ...
    t4 = this.box_0;
    t5 = t1.Y;
    t2.plot_points$3(C.JSArray_methods.sublist$2(t3, 0, t4.i_0), C.JSArray_methods.sublist$2(t5, 0, t4.i_0), "blue");
    t6 = this.box_1;
    t2.plot_points$3(C.JSArray_methods.sublist$2(t3, t4.i_0, t6.j_1 + 1), C.JSArray_methods.sublist$2(t5, t4.i_0, t6.j_1 + 1), "red");
    t2.plot_points$3(C.JSArray_methods.sublist$1(t3, t6.j_1 + 1), C.JSArray_methods.sublist$1(t5, t6.j_1 + 1), "blue");
    t5 = this.cell_4;
    J.set$backgroundColor$x(J.get$style$x(t5), "red");
    t3 = t1.lines;
    t7 = t4.i_0;
    ...
    if (t3 < 0 || t3 >= t7.length)
      throw H.ioore(t7, t3);
    t5.textContent = C.JSNumber_methods.toStringAsFixed$1(t7[t3].get$sse(), 2);
    t3 = t4.i_0;
    t6 = t6.j_1;
    if (t3 !== t6) {
      t1 = t1.lines;
      ...
      if (t6 < 0 || t6 >= t3.length)
        throw H.ioore(t3, t6);
      t2.plot_function$2(t3[t6].to_function$0(), "red");
    }
  }
},

SLSAnimation_error_animation_closure1: {"": "Closure;box_0,this_5,cell_6",
  call$0: function() {
    var t1, t2, t3, t4;
    J.set$backgroundColor$x(J.get$style$x(this.cell_6), "#F8F8F8");
    t1 = this.this_5;
    t2 = t1.sls;
    t3 = t2.X;
    t4 = this.box_0.i_0;
    ...
    if (t4 >= t2.length)
      throw H.ioore(t2, t4);
    t1.plot.plot_point$3(t3, t2[t4], "blue");
  }
},

SLSAnimation_optimal_animation_closure: {"": "Closure;box_1,this_2",
  call$0: function() {
    var t1, t2, t3, t4;
    t1 = this.this_2;
    t2 = t1.sls;
...
SLSAnimation_optimal_animation_closure0: {"": "Closure;box_0,box_1,this_3,cost_cell_4,previous_cost_cell_5,error_cell_6",
  call$0: function() {
    ...
    t2.plot_points$3(C.JSArray_methods.sublist$2(t3, 0, t4.i_0), C.JSArray_methods.sublist$2(t5, 0, t4.i_0), "blue");
    t6 = this.box_1;
    t2.plot_points$3(C.JSArray_methods.sublist$2(t3, t4.i_0, t6.j_1 + 1), C.JSArray_methods.sublist$2(t5, t4.i_0, t6.j_1 + 1), "green");
    t2.plot_points$3(C.JSArray_methods.sublist$1(t3, t6.j_1 + 1), C.JSArray_methods.sublist$1(t5, t6.j_1 + 1), "blue");
    J.set$backgroundColor$x(J.get$style$x(this.previous_cost_cell_5), "red");
    t5 = this.cost_cell_4;
    J.set$backgroundColor$x(J.get$style$x(t5), "green");
    t1 = t1.cost;
    t4 = t4.i_0;
    ...
    if (t6 < 0 || t6 >= t4.length)
      throw H.ioore(t4, t6);
    t5.textContent = J.toStringAsFixed$1$n(t4[t6], 2);
    J.set$backgroundColor$x(J.get$style$x(this.error_cell_6), "red");
  }
},

SLSAnimation_optimal_animation_closure1: {"": "Closure;box_0,this_7,cost_cell_8,previous_cost_cell_9,error_cell_10",
  call$0: function() {
    var t1, t2, t3, t4;
    J.set$backgroundColor$x(J.get$style$x(this.previous_cost_cell_9), "#F8F8F8");
    J.set$backgroundColor$x(J.get$style$x(this.cost_cell_8), "#F8F8F8");
    J.set$backgroundColor$x(J.get$style$x(this.error_cell_10), "#F8F8F8");
    t1 = this.this_7;
    t2 = t1.sls;
    t3 = t2.X;
    t4 = this.box_0.i_0;
...
SLSAnimation_optimal_animation_closure2: {"": "Closure;box_1,this_11,opt_cell_12",
  call$0: function() {
    var t1, t2, t3;
    t1 = this.opt_cell_12;
    J.set$backgroundColor$x(J.get$style$x(t1), "green");
    t2 = this.this_11.sls.M;
    t3 = this.box_1.j_1 + 1;
    if (t3 < 0 || t3 >= t2.length)
      throw H.ioore(t2, t3);
    t1.textContent = J.toStringAsFixed$1$n(t2[t3], 2);
  }
},

SLSAnimation_optimal_animation_closure3: {"": "Closure;opt_cell_13",
  call$0: function() {
    J.set$backgroundColor$x(J.get$style$x(this.opt_cell_13), "#F8F8F8");
  }
},

SLSAnimation_plot_solution_closure: {"": "Closure;this_0,equation_list_1,X0_2,Y0_3,line_4,opt_cell_5,cost_cell_6,i_cell_7,j_cell_8,color_9",
  call$0: function() {
    var t1, t2, t3, equation;
    t1 = this.color_9;
    J.set$backgroundColor$x(J.get$style$x(this.opt_cell_5), t1);
    J.set$backgroundColor$x(J.get$style$x(this.cost_cell_6), t1);
    J.set$backgroundColor$x(J.get$style$x(this.i_cell_7), t1);
    J.set$backgroundColor$x(J.get$style$x(this.j_cell_8), t1);
    t2 = this.this_0.plot;
    t2.plot_points$3(this.X0_2, this.Y0_3, t1);
    t3 = this.line_4;
    t2.plot_function$2(t3.to_function$0(), t1);
    equation = document.createElement("li", null);
    J.set$color$x(equation.style, t1);
    equation.textContent = t3.toString$0(t3);
    this.equation_list_1.appendChild(equation);
  }
}},
1],
]);
...
F.LeastSquaresLine.$isObject = true;
...
//# sourceMappingURL=sls.dart.js.map
//@ sourceMappingURL=sls.dart.js.map
